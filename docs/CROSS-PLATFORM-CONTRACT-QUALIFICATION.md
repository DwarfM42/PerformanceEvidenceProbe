...
| `target.json` exit/path, host/config/capabilities metadata | Platform-owned contextual metadata. | Linux must state its actual authority and scope. Missing optional target exit is allowed; unavailable metadata must not be replaced by a false Windows-shaped value. |
...
| Target exit / disappearance | Windows retained handles can observe an exit code; Linux attach does not own `waitpid` exit status. Root may die while descendants live in Windows launch. | Exit fields are optional. Finalization can use the last actual sample; no final live sample is required. A complete bundle requires at least one complete sample record; an interruption/no-sample path stays raw-only. |
...
| Run / launch | Windows starts suspended, assigns a non-destructive Job, verifies membership, then resumes. Linux has no Job Object equivalence. | Future Linux run must record its launch/ownership and child-accounting authority as platform-specific metadata. It may omit `job`; it must not label a process group/cgroup as Windows Job accounting. Target launch failure before identity remains no completed bundle under the current contract. |
